...
namespace xTile
{
	...
	public class Map : DescribedComponent
	{
		...
		/// </summary>
		public ReadOnlyCollection<Layer> Layers => m_layers.AsReadOnly();

		/// <summary>
		/// TileSheet collection in the Map
		/// </summary>
		public ReadOnlyCollection<TileSheet> TileSheets => m_tileSheets.AsReadOnly();
		...
		{
			var layerIndex = m_layers.IndexOf(layer);
			if (layerIndex < 0)
				throw new Exception("The specified Layer is not contained in this map");
		...
		{
			foreach (var tileSheet in m_tileSheets)
		...
		public void AddTileSheet(TileSheet tileSheet)
		{
			...
			m_tileSheets.Sort(
				(tileSheet1, tileSheet2) => tileSheet1.Id.CompareTo(tileSheet2.Id));
		}
		...
		{
			var deviceViewport = new Rectangle(displayOffset, mapViewport.Size);
			...
			foreach (var layer in m_layers)
			{
				...
				var layerViewport = layer.ConvertMapToLayerViewport(mapViewport);
		...
		internal void UpdateDisplaySize()
		{
			...
			foreach (var layer in m_layers)
			{
				var displaySize = layer.DisplaySize;
